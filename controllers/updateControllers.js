...
export const getDonors = async (req, res) => {
  ...
  if (bloodBank) {
    ...
    const name = bloodBank.name;

    res.status(200).json({ donors, name });
  } else {
...
export const update = async (req, res) => {
  ...
  const existingDonor = await Donor.findOne({ email }).select(
    " -password -donatedAt -createdAt -updatedAt -__v"
  );
...
export const createDonor = async (req, res) => {
  ...
  if (existingDonor) {
    ...
    res
      ...
      .json({ msg: "donor added to donors list", bloodbank });
  } else {
    const donorToadd = req.body;
    donorToadd.date = new Date();
    await BloodBank.findOneAndUpdate(
    ...
    res.status(StatusCodes.OK).json({
      msg: "donor registered successfully and added to donors list",
      bloodbank,
    });
  }
};
